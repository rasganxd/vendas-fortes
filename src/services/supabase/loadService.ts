...
class LoadSupabaseService {
  async generateNextCode(): Promise<number> {
    try {
      const { data, error } = await supabase.rpc('get_next_load_code');
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      console.error('Error in delete load:', error);
      throw error;
    }
  }

  async getById(id: string): Promise<Load | null> {
    try {
      const { data, error } = await supabase
        .from('loads')
        .select('*')
