...
interface UseProductValidationFixedProps {
  products: Product[];
}

export function useProductValidationFixed({ products }: UseProductValidationFixedProps) {
  ...
  const validateProductDiscount = useCallback((productId: string, discountedPrice: number): string | boolean => {
    const product = products.find(p => p.id === productId);
    if (!product) return "Produto não encontrado";
