      ...
      <div className="px-4 py-3 border-b bg-gray-50">
        <div className="flex items-center justify-between">
          ...
          {orderItems.length > 3 && <div className="relative">
              ...
              <Input type="text" placeholder="Filtrar itens..." value={searchFilter} onChange={e => setSearchFilter(e.target.value)} className="pl-9 w-64 h-8 text-sm" />
            </div>}
        </div>
        
        {isEditMode && <div className="mt-2 text-sm text-blue-600">
            💡 Clique no ícone de edição para alterar quantidade ou preço
          </div>}
      </div>

      {filteredItems.length === 0 ? <div className="text-center py-16 text-gray-500">
          {searchFilter ? <div>
          ...
          <table className="w-full border-collapse">
            ...
            return <tr key={itemKey} className="border-t hover:bg-gray-50 transition-colors">
                ...
                }))} className="w-20 h-8 text-center" min="0" step="1" /> : <span className="font-semibold text-gray-800">{item.quantity}</span>}
                    </td>
                    <td className="px-4 py-4 text-center">
                  ...
                  setEditValues(prev => ({
                    ...
                    <td className="px-4 py-4 text-center">
                      <div className="flex items-center justify-center space-x-1">
                        {isEditing ? <>
                            <Button variant="ghost" size="sm" onClick={() => saveEditing(item)} className="hover:bg-green-50 hover:text-green-600" title="Salvar alterações">
                              <Check size={16} />
                            </Button>
                            <Button variant="ghost" size="sm" onClick={cancelEditing} className="hover:bg-gray-50 hover:text-gray-600" title="Cancelar edição">
                              <X size={16} />
                            </Button>
                          </> : <>
                            {isEditMode && <Button variant="ghost" size="sm" onClick={() => startEditing(item)} className="hover:bg-blue-50 hover:text-blue-600" title="Editar item">
                                <Edit2 size={16} />
                              </Button>}
                            <Button variant="ghost" size="sm" onClick={() => handleRemoveClick(item, index)} type="button" className="hover:bg-red-50 hover:text-red-600" title={`Remover ${item.productName}`}>
                              <Trash2 size={16} />
                            </Button>
                          </>}
                      </div>
