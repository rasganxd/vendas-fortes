...
export default function EnhancedProductSearch({
  ...
  const filteredProducts = products.filter(product => product.name.toLowerCase().includes(productSearch.toLowerCase())).slice(0, 20);
  ...
  const handleUnitChange = (unit: string) => {
    ...
    const convertedPrice = converter.calculateUnitPrice(basePrice, 1,
    // quantidade 1
    unit,
    // unidade selecionada (ex: "UN")
  ...
  const getCurrentUnitPrice = (): number => {
    ...
    const basePrice = selectedProduct.sale_price || selectedProduct.price || 0;
    return converter.calculateUnitPrice(basePrice, 1,
    // quantidade 1
    selectedUnit,
    // unidade selecionada
  ...
  return <div className="space-y-4">
      ...
      <ProductCodeInput products={products} foundProduct={foundProduct} onProductFound={setFoundProduct} onProductSelect={handleProductSelect} onSearchDialogOpen={() => setIsProductSearchOpen(true)} inputRef={productInputRef} isEditMode={isEditMode} />
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {(selectedProduct.maxDiscountPercent || selectedProduct.max_discount_percent) && <div className="text-xs text-orange-600">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <Input ref={priceInputRef} type="text" value={priceDisplayValue} onChange={handlePriceChange} onKeyDown={handlePriceKeyDown} placeholder="0,00" className={`h-10 ${priceValidation?.type === 'error' ? 'border-red-500 focus:border-red-500' : priceValidation?.type === 'warning' ? 'border-yellow-500 focus:border-yellow-500' : 'border-green-500 focus:border-green-500'}`} disabled={!canEnterPrice} />
                  {priceValidation && <div className={`flex items-center gap-1 mt-1 text-xs ${priceValidation.type === 'error' ? 'text-red-600' : priceValidation.type === 'warning' ? 'text-yellow-600' : 'text-green-600'}`}>
                ...
                <div>
                  ...
                  <QuantityInput quantity={canEnterQuantity ? quantity : null} onQuantityChange={e => setQuantity(parseInt(e.target.value) || 1)} onIncrement={() => setQuantity(prev => prev + 1)} onDecrement={() => setQuantity(prev => Math.max(1, prev - 1))} inputRef={quantityInputRef} onKeyDown={handleQuantityKeyDown} disabled={!canEnterQuantity} />
                </div>

                <div className="flex items-end">
                  <Button ref={addButtonRef} onClick={handleAdd} className="w-full h-10 bg-green-600 hover:bg-green-700" disabled={!canAdd}>
              ...
              {getQuantityConversion() && <div className="text-xs text-gray-600">
      ...
      <ProductSearchDialog open={isProductSearchOpen} onOpenChange={setIsProductSearchOpen} products={filteredProducts} productSearch={productSearch} onSearchChange={setProductSearch} onSelectProduct={handleProductSelect} />
    </div>;
}