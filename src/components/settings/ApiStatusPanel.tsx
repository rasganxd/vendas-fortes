...
import { Wifi, WifiOff, RefreshCw, CheckCircle, XCircle, AlertCircle, Globe, Key } from "lucide-react";
...
const ApiStatusPanel: React.FC = () => {
  ...
  const getStatusBadge = () => {
    ...
    return apiStatus.isConnected ? <Badge variant="default" className="bg-green-500">Conectado</Badge> : <Badge variant="destructive">Desconectado</Badge>;
  };
  return <Card>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {apiStatus.lastChecked && <div className="text-sm space-y-1">
            <p>
              <strong>Última verificação:</strong> {apiStatus.lastChecked.toLocaleString()}
            </p>
            {apiStatus.responseTime && <p>
        ...
        {/* Erro */}
        {apiStatus.error && <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
        ...
        <div className="flex gap-2">
          <Button onClick={() => testApiConnection(true)} disabled={isTestingConnection} variant="outline" className="flex-1">
            {isTestingConnection ? <>
          ...
          <Button onClick={testWithAuthentication} disabled={isTestingConnection} variant="outline" className="flex-1">
            {isTestingConnection ? <>
                <LoadingSpinner size="sm" className="mr-2" />
                Testando...
              </> : <>
