...
export default function PromissoryNotesTab({
  ...
  } = useOrdersDateFilter(promissoryOrders);
  ...
  const filteredOrders = selectedOrderId ? dateFilteredOrders.filter(order => order.id === selectedOrderId) : dateFilteredOrders;
  ...
  const handlePrintSelectedNotes = async () => {
    ...
    const ordersToPrint = promissoryOrders.filter(order => selectedOrderIds.includes(order.id));

    // Check if company data is loaded
    const companyData = settings?.company;
    const isCompanyDataLoaded = companyData && companyData.name && companyData.name !== 'Carregando...' && companyData.name.trim() !== '';
  ...
  const handleElectronPrint = async (ordersToPrint: any[]) => {
    try {
      ...
      const htmlContent = generateMultiplePromissoryNotesHTML(ordersToPrint, customers, paymentTables, payments, settings?.company);
  ...
  const selectedOrdersInFiltered = selectedOrderIds.filter(id => filteredOrders.some(order => order.id === id));
  ...
  return <div>
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            <Checkbox checked={allFilteredSelected} onCheckedChange={handleSelectAllOrders} className="mr-2" />
            <span className="text-sm font-medium">
              Selecionar todas visíveis ({selectedOrdersInFiltered.length} de {filteredOrders.length})
            </span>
          </div>
          
          {selectedOrderIds.length > 0 && <Button onClick={handlePrintSelectedNotes} className="bg-blue-600 hover:bg-blue-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <OrdersDateFilter selectedPeriod={selectedPeriod} onPeriodChange={setSelectedPeriod} periodCounts={periodCounts} />
          </div>

          {/* Order Specific Filter */}
          <div>
            <label className="block text-sm font-medium mb-2">Filtrar por Pedido</label>
            <Select value={selectedOrderId || 'all'} onValueChange={value => setSelectedOrderId(value === 'all' ? null : value)}>
              ...
              <SelectContent>
                ...
                {dateFilteredOrders.map(order => <SelectItem key={order.id} value={order.id}>
        ...
        {/* Filter Summary */}
        {(selectedPeriod !== 'all' || selectedOrderId) && <div className="text-sm text-gray-600 mb-4 p-3 bg-blue-50 rounded-md border border-blue-200">
            <div className="flex items-center gap-4">
              <span>
                📊 Filtros ativos: 
                {selectedPeriod !== 'all' && <span className="ml-1 px-2 py-1 bg-blue-100 rounded text-xs">
                ...
                {selectedOrderId && <span className="ml-1 px-2 py-1 bg-green-100 rounded text-xs">
                    Pedido específico
                  </span>}
              </span>
              <span>
      ...
      <div className="space-y-10">
        ...
        const paymentTable = paymentTables.find(pt => pt.id === order.paymentTableId);
        return <div key={order.id} id={`promissory-note-${order.id}`} className="bg-white p-4 border rounded-md shadow transition-all">
              <div className="flex items-center mb-4">
                <Checkbox checked={selectedOrderIds.includes(order.id)} onCheckedChange={() => handleToggleOrderSelection(order.id)} className="mr-3" />
                <h3 className="text-lg font-semibold">
                  Nota Promissória - Pedido #{order.code}
                </h3>
              </div>
              
              <PromissoryNoteView order={order} customer={customer} paymentTable={paymentTable} payments={payments.filter(p => p.orderId === order.id)} />
            </div>;
      })}
      </div>
    </div>;
