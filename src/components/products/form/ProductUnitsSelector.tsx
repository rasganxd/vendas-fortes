...
export const ProductUnitsSelector: React.FC<ProductUnitsSelectorProps> = ({
  ...
  const availableUnits = allUnits.filter(unit => !selectedUnits.some(su => su.unitId === unit.id));
  console.log("📋 Available units:", availableUnits.map(u => ({
    id: u.id,
    value: u.value
  })));
  const handleAddUnit = () => {
  ...
  return <Card className={className}>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" variant="outline" type="button" onClick={() => setAddUnitDialogOpen(true)} disabled={availableUnits.length === 0}>
      ...
      <CardContent>
        {selectedUnits.length === 0 ? <p className="text-sm text-muted-foreground">
            Nenhuma unidade selecionada. Adicione pelo menos uma unidade.
          </p> : <div className="space-y-2">
            {selectedUnits.map(unit => <div key={unit.unitId} className="flex items-center justify-between p-2 border rounded-md">
                <div className="flex items-center gap-2">
                  <span className="font-medium">{unit.unitValue}</span>
                  <span className="text-sm text-muted-foreground">
                    {unit.unitLabel}
                  </span>
                  {unit.isMainUnit && <Badge variant="default" className="text-xs">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!unit.isMainUnit && <Button size="sm" variant="outline" type="button" onClick={() => handleSetMainUnit(unit.unitId)} className="h-6 px-2 text-xs">
                      Definir como principal
                    </Button>}
                  <Button size="sm" variant="outline" type="button" onClick={() => handleRemoveUnit(unit.unitId)} className="h-6 w-6 p-0" disabled={selectedUnits.length === 1}>
      ...
      <Dialog open={addUnitDialogOpen} onOpenChange={setAddUnitDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            {availableUnits.length === 0 && <p className="text-sm text-muted-foreground">
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={handleAddUnit} disabled={!selectedUnitId || availableUnits.length === 0}>
              Adicionar
            </Button>
          </DialogFooter>
        </DialogContent>
